...
def compress(zip_name:str = getcwd().replace("\\", "/").split("/")[-1], wanted:list = [], path:str = "", is_first:bool = True, file:zipfile.ZipFile|None = None, init_path:str = "",compress_files: bool = False):
	...
	if is_first:
		init_path = path
		all_folder = include_in(wanted,listdir(path))
	...
	if is_first:
		file.close()
		if compress_files: open("temp.txt","w").close()

start(path="Datapack",wanted=["pack.mcmeta","data"],compress_files=True)
start(path="Resourcepack",zip_name="Sorcery Maker RP",wanted=["pack.mcmeta","assets"],compress_files=False)
